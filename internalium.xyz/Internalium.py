...
def CellButton(frame):
    b=Button(frame,padx=1,bg="Powder Blue",width=3,text="",font=(r ,60,'bold'),relief="sunken",bd=3)
    return b
def changeLetter():
    global a
    for i in ['O','X']:
...
Label(root,text="Nama Player Pertama[X]",bg="Navy",fg="white",font=("Arial",15,"bold")).place(x=550,y=20)
#textbox for first player
Entry(root,font=("Arial",15,"bold"),textvariable=firstPlayer).place(x=550,y=55)
#label for second player
Label(root,text="Nama Player Kedua[O]",bg="Navy",fg="white",font=("Arial",15,"bold")).place(x=550,y=90)
...
lblMesage=Label(root,bg="Navy",fg="yellow",font=("Arial",15,"bold"))
lblMesage.place(x=550,y=185)
#Defining Two operators
a=r.choice(['O','X'])
#setting color for operators
colour={'O':"Cyan",'X':"Cadet Blue"}
...
label=Label(text="",font=('arial',15,'bold'),bg="Navy",fg="white")
